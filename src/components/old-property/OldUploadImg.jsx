...
export default function OldUploadImg({imageFiles, setImageFiles}) {
    ...
    launchImageLibrary(
      ...
      async response => {
        ...
        try {
          ...
          // ── Store only the S3 URL ──
          setImageFiles(prev => ({
            ...prev,
            [selectedSection]: s3Url, // string URL, not file object
          }));
        } catch (err) {
          clearInterval(interval);
  ...
  useEffect(() => {
    const hasAnyImage = Object.values(imageFiles).some(v => !!v);
...
const styles = StyleSheet.create({
  ...
  // uploaded-but-not-selected pill goes green
  doneSection: {
